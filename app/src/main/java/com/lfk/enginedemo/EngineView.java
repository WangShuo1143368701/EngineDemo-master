...
public abstract class EngineView extends FrameLayout implements Runnable, View.OnTouchListener {
    ...
    public EngineView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public EngineView(Context context, boolean isOpenDebug) {
        super(context);
        this.mContext = context;
        if (!isOpenDebug) {
            Logger.init().logLevel(LogLevel.NONE);
...
//        setScreenOrientation(ScreenMode.LANDSCAPE);
        ...
        // init surfaceView
        e_surfaceView = new SurfaceView(mContext);

        // set content view
        addView(e_surfaceView);
...
//                    Logger.e("engine run start error:" + e);
        ...
        System.exit(-1);// RESULT_OK waangshuo ?
    }
